    ...
    if (strtolower($status) === 'paid' || $eventType === 'transaction_paid') {
        ...
        if ($pedido) {
            ...
            if (!empty($payerInfo)) {
                ...
                error_log("[Webhook QuantumPay] 💳 Informações do pagador PIX adicionadas");
            }
            ...
            error_log("[Webhook QuantumPay] 📤 Resposta do utmify (HTTP $httpCode): " . $utmifyResponse);
            if ($curlError) {
                error_log("[Webhook QuantumPay] ❌ Erro ao enviar para utmify: " . $curlError);
            } else {
                error_log("[Webhook QuantumPay] 📊 Resposta decodificada: " . print_r(json_decode($utmifyResponse, true), true));
            }
