...
try {
    // Configurações da API Sunize
    $apiUrl = 'https://api.sunize.com.br/v1/transactions';
    $apiKey = 'ck_0ad3023a72d7b82aa1ce05bf3c0ea114'; // API Key da Sunize
    $apiSecret = 'cs_17fa0e3e5a7409b1f8705e30bd0f22d3'; // API Secret da Sunize
    ...
    $externalId = 'SUNIZE-' . time() . '-' . rand(1000, 9999);

    error_log("[Sunize] 📝 Preparando dados para envio: " . json_encode([
        'valor' => $valor,
    ...
    // Formata telefone para padrão E.164 (+5511999999999)
    $telefoneFormatado = '+55' . preg_replace('/\D/', '', $telefone);
    
    // Converte valor de centavos para reais (formato decimal)
    $valorReais = $valor_centavos / 100;

    // Estrutura do payload conforme documentação Sunize
    $data = [
        "external_id" => $externalId,
    ...
    // Adiciona splits se necessário (opcional)
    // $data['splits'] = [
    //     [
    ...
    error_log("[Sunize] 🌐 URL da requisição: " . $apiUrl);
    error_log("[Sunize] 📦 Dados enviados: " . json_encode($data));
    ...
    error_log("[Sunize] 🔍 Detalhes da requisição cURL:\n" . $verboseLog);

    if ($curlError) {
        error_log("[Sunize] ❌ Erro cURL: " . $curlError . " (errno: " . $curlErrno . ")");
        throw new Exception("Erro na requisição: " . $curlError);
    }

    curl_close($ch);

    error_log("[Sunize] 📊 HTTP Status Code: " . $httpCode);
    error_log("[Sunize] 📄 Resposta bruta: " . $response);

    // Aceita códigos de sucesso (200-299)
    if ($httpCode < 200 || $httpCode >= 300) {
        $errorMsg = "Erro na API Sunize: HTTP " . $httpCode;
        if (!empty($response)) {
            $errorMsg .= " - " . $response;
        }
    ...
    if (isset($result['hasError']) && $result['hasError'] === true) {
        $errorMsg = isset($result['message']) ? $result['message'] : 'Erro desconhecido na API Sunize';
        throw new Exception("Erro na API Sunize: " . $errorMsg);
    }

    if (!isset($result['id'])) {
        throw new Exception("Transaction ID não encontrado na resposta da API Sunize");
    }

    // Usa transaction_id da Sunize
    ...
    error_log("[Sunize] 💳 Transação criada com sucesso: " . $transactionId);
    error_log("[Sunize] 📄 Resposta completa da API: " . $response);
    error_log("[Sunize] 🔑 Token gerado: " . $transactionId);
    error_log("[Sunize] 📊 Status da transação: " . ($result['status'] ?? 'N/A'));
    ...
    // Extrai o código PIX da resposta Sunize
    $pixCode = $result['pix']['payload'] ?? null;
    
    if (empty($pixCode)) {
        error_log("[Sunize] ⚠️ Código PIX não encontrado na resposta");
    }

    // Gera URL do QR Code usando o payload PIX
    $qrCodeUrl = null;
    if (!empty($pixCode)) {
        $qrCodeUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' . urlencode($pixCode);
        error_log("[Sunize] 🔄 QR Code gerado via API externa: " . $qrCodeUrl);
    }
    ...
    error_log("[Sunize] 📤 Enviando resposta ao frontend: " . json_encode($responseData));
    echo json_encode($responseData);

} catch (Exception $e) {
    error_log("[Sunize] ❌ Erro: " . $e->getMessage());
    error_log("[Sunize] 🔍 Stack trace: " . $e->getTraceAsString());
