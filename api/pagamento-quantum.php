...
try {
    // Configurações da API QuantumPay
    $apiKey = 'cmmxjjj4t000313f50phn0ze5'; // API Key (Pública)
    $apiSecret = ''; // API Secret (Secreta)
    
    // URLs da API QuantumPay
    $authUrl = 'https://api.quantumpay.com.br/api/auth';
    $pixUrl = 'https://api.quantumpay.com.br/api/v1/pix/in/qrcode';
    
    error_log("[QuantumPay] 🔑 API Key: " . substr($apiKey, 0, 10) . "...");
    error_log("[QuantumPay] 🔐 API Secret (length): " . strlen($apiSecret) . " chars");
    ...
    // ===== PASSO 1: AUTENTICAÇÃO - Gerar Token Bearer =====
    // Verifica se já existe um token válido em cache
    $tokenCacheFile = __DIR__ . '/quantum-token-cache.json';
    $bearerToken = null;
    $needNewToken = true;
    
    if (file_exists($tokenCacheFile)) {
        $tokenCache = json_decode(file_get_contents($tokenCacheFile), true);
        
        if ($tokenCache && isset($tokenCache['token']) && isset($tokenCache['expires_at'])) {
            // Verifica se o token ainda é válido (com margem de 5 minutos)
            $expiresAt = $tokenCache['expires_at'];
            $now = time();
            
            if ($expiresAt > ($now + 300)) { // 5 minutos de margem
                $bearerToken = $tokenCache['token'];
                $needNewToken = false;
                error_log("[QuantumPay] ✅ Usando token em cache (válido até " . date('Y-m-d H:i:s', $expiresAt) . ")");
            } else {
                error_log("[QuantumPay] ⚠️ Token em cache expirado, gerando novo...");
            }
        }
    }
    ...
    if ($needNewToken) {
        error_log("[QuantumPay] 🔐 Iniciando autenticação...");
        error_log("[QuantumPay] 🌐 URL de autenticação: " . $authUrl);
        
        // Gera credenciais em Base64 para Basic Auth
        $credentials = base64_encode($apiKey . ':' . $apiSecret);
        
        error_log("[QuantumPay] 🔑 Credentials Base64 (primeiros 50 chars): " . substr($credentials, 0, 50) . "...");
        
        $authCh = curl_init($authUrl);
        curl_setopt_array($authCh, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => '', // Body vazio
            CURLOPT_HTTPHEADER => [
                'Authorization: Basic ' . $credentials,
                'Content-Length: 0' // Header obrigatório conforme documentação
            ],
            ...
            CURLOPT_USERAGENT => 'QuantumPay-PHP-Client/1.0'
        ]);
        
        // Captura verbose output
        $verboseAuth = fopen('php://temp', 'w+');
        curl_setopt($authCh, CURLOPT_STDERR, $verboseAuth);
        ...
        // Log verbose
        rewind($verboseAuth);
        $verboseAuthLog = stream_get_contents($verboseAuth);
        error_log("[QuantumPay] 🔍 Detalhes da autenticação cURL:\n" . $verboseAuthLog);
        
        curl_close($authCh);
        
        if ($authError) {
            error_log("[QuantumPay] ❌ Erro cURL na autenticação: " . $authError);
            throw new Exception("Erro na autenticação (cURL): " . $authError);
        }
        
        error_log("[QuantumPay] 📊 Status autenticação: " . $authHttpCode);
        error_log("[QuantumPay] 📄 Resposta autenticação: " . $authResponse);
        
        if (empty($authResponse)) {
            throw new Exception("Resposta vazia da API de autenticação (HTTP " . $authHttpCode . ")");
        }
        
        if ($authHttpCode !== 200 && $authHttpCode !== 201) {
            error_log("[QuantumPay] ❌ HTTP Code inválido: " . $authHttpCode);
            throw new Exception("Erro na autenticação: HTTP " . $authHttpCode . " - " . $authResponse);
        }
        
        $authResult = json_decode($authResponse, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            error_log("[QuantumPay] ❌ Erro JSON: " . json_last_error_msg());
            throw new Exception("Erro ao decodificar resposta de autenticação: " . json_last_error_msg());
        }
        
        // Extrai o token conforme documentação
        if (isset($authResult['token'])) {
            $bearerToken = $authResult['token'];
        } elseif (isset($authResult['data']['token'])) {
            $bearerToken = $authResult['data']['token'];
        }
        
        if (empty($bearerToken)) {
            error_log("[QuantumPay] ❌ Token não encontrado. Resposta: " . json_encode($authResult));
            throw new Exception("Token não encontrado na resposta de autenticação");
        }
        
        // Calcula quando o token expira (30 minutos conforme documentação)
        $expiresIn = $authResult['expiresIn'] ?? 1800000; // 30 minutos em ms
        $expiresAt = time() + ($expiresIn / 1000); // Converte ms para segundos
        ...
        file_put_contents($tokenCacheFile, json_encode($tokenCache, JSON_PRETTY_PRINT));
        
        error_log("[QuantumPay] ✅ Token gerado e salvo em cache");
        error_log("[QuantumPay] ⏰ Token válido até: " . date('Y-m-d H:i:s', $expiresAt));
    }
    
    // ===== PASSO 2: CRIAR QR CODE PIX =====
    error_log("[QuantumPay] 📝 Preparando dados para criação do PIX...");
    
    // Monta URL do postback (webhook)
    $serverUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
    $currentDir = dirname($_SERVER['REQUEST_URI']);
    $postbackUrl = $serverUrl . $currentDir . "/webhook-quantum.php";
    
    // Estrutura do payload conforme documentação QuantumPay
    $data = [
        "amountInCents" => $valor_centavos,
    ...
    error_log("[QuantumPay] 🌐 URL da requisição PIX: " . $pixUrl);
    error_log("[QuantumPay] 📦 Dados enviados: " . json_encode($data));
    ...
    error_log("[QuantumPay] 🔄 Redirecionamentos: " . $redirectCount);
    error_log("[QuantumPay] 🎯 URL final: " . $effectiveUrl);

    // Log detalhado do cURL
    rewind($verbose);
    $verboseLog = stream_get_contents($verbose);
    error_log("[QuantumPay] 🔍 Detalhes da requisição cURL:\n" . $verboseLog);

    if ($curlError) {
        error_log("[QuantumPay] ❌ Erro cURL: " . $curlError . " (errno: " . $curlErrno . ")");
        throw new Exception("Erro na requisição: " . $curlError);
    }

    curl_close($ch);

    error_log("[QuantumPay] 📊 HTTP Status Code: " . $httpCode);
    error_log("[QuantumPay] 📄 Resposta bruta: " . $response);
    ...
    // Usa transaction_id da QuantumPay
    $transactionId = $result['data']['id'];
    $pixEmv = $result['data']['pix']['emv'] ?? null;
    $qrCodeBase64 = $result['data']['pix']['qrCode'] ?? null;
    ...
    error_log("[QuantumPay] 💳 Transação criada com sucesso: " . $transactionId);
    error_log("[QuantumPay] 📄 Resposta completa da API: " . $response);
    error_log("[QuantumPay] 🔑 Token gerado: " . $transactionId);
   ...
   $serverUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
    ...
    // Se tiver QR Code em Base64, usa diretamente
    if (!empty($qrCodeBase64)) {
        // Se já tiver o prefixo data:image, usa direto, senão adiciona
        if (strpos($qrCodeBase64, 'data:image') === 0) {
            $qrCodeUrl = $qrCodeBase64;
        } else {
            $qrCodeUrl = 'data:image/png;base64,' . $qrCodeBase64;
        }
        error_log("[QuantumPay] ✅ QR Code Base64 recebido da API");
    }
    // Se não tiver QR Code e tiver o código PIX EMV, gera via API pública
    elseif (!empty($pixEmv)) {
        $qrCodeUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' . urlencode($pixEmv);
        error_log("[QuantumPay] 🔄 QR Code gerado via API externa");
    }
    ...
    $responseData = [
        ...
        'token' => $transactionId,
        'pixCode' => $pixEmv,
        'pixCopiaECola' => $pixEmv, // Adiciona o campo que o frontend espera
    ...
    error_log("[QuantumPay] 📤 Enviando resposta ao frontend: " . json_encode($responseData));
    echo json_encode($responseData);

} catch (Exception $e) {
    error_log("[QuantumPay] ❌ Erro: " . $e->getMessage());
    error_log("[QuantumPay] 🔍 Stack trace: " . $e->getTraceAsString());
